...
public class ClientUpgrade {
	...
	private class CheckVersionAsyncTask extends
		...
		protected void onPostExecute(String result) {
			System.out.println("result::::::::::::version" + result);
	...
	public void showUpgradeTipsDialog(String message, boolean force) {
		final Dialog alertDialog = new AlertDialog.Builder(_context)
				.setMessage(message)
		...
		// v.findViewById(R.id.dialog_text_cancel);
		//
		...
		// }
		//
		// int w = Utils.dip2px(_context, 350);
		// int h = Utils.dip2px(_context, 159);
		// dlg.setContentView(
		// v,
		// new LinearLayout.LayoutParams(
		// Utils.getSecreenWidth(_context) - 100, Utils
		// .getSecreenWidth(_context) / 2));
		// dlg.show();
	}
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case DOWN_ERROR:
				_downloadDialog.dismiss();
				showAlertDialog(_context, "提示", "下载失败！");
				break;
			default:
				break;
			}
	...
	private int installApk() {
		...
		Utils.execCmd("chmod 777 " + file.toString());
	...
	private void showDownloadDialog() {
		...
		_downloadDialog = new AlertDialog.Builder(_context).setView(v)
				.setNegativeButton("取消", new DialogInterface.OnClickListener() {
		...
		// final Dialog dlg = new Dialog(_context, R.style.dialog);
		// View v =
		// LayoutInflater.from(_context).inflate(R.layout.dialog_upgrade,
		// null);
		//
		// _progressBar = (ProgressBar) v.findViewById(R.id.progressBar1);
		// _titleTextView = (TextView) v.findViewById(R.id.textView);
		...
		//
		// int w = Utils.dip2px(_context, 350);
		// int h = Utils.dip2px(_context, 180);
		//
	...
	public String checkVersion() {
		try {
			String currentVersion = Utils.getVersionCode(_context);
